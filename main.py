...
def home():
    if request.method == 'POST':
        ...
        if not symptoms or symptoms.lower() == "symptoms":
            message = "⚠️ Please enter your symptoms (comma-separated) before submitting."
        ...
        try:
            ...
            # Unpack nested arrays properly
            my_precautions = precautions_list[0] if precautions_list else []
            medications_list = medications_list or []
            rec_diet = rec_diet or []
            workout_list = list(workout_list) if workout_list is not None else []
            ...
            message = f"⚠️ Something went wrong during prediction: {str(e)}"
            return render_template('index.html', message=message)
